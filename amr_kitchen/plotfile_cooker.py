...
class PlotfileCooker(object):
            ...
            try:
                ...
                if validate_mode:
                    ...
                    raise TastesBadError(
                        (
                            f"PlotfileCooker encountered a fatal"
    ...
    def read_cell_headers(self, maxmins, validate_mode):
        ...
        for i in range(self.limit_level + 1):
            ...
            if maxmins:
                lvcells["mins"] = {}
                lvcells["maxs"] = {}
                for field, minvals, maxvals in zip(
                    self.fields, np.transpose(lvmins), np.transpose(lvmaxs)
                ):
    ...
    def byboxcompared(self, other, lv):
        ...
        """
        for bf1, bf2, off1, off2, idxs in zip(
            self.cells[lv]["files"],
    ...
    def writehdrnewboxes(self, pfdir, boxes, fields):
        ...
        with open(os.path.join(os.getcwd(), pfdir, "Header"), "w") as hfile:
            ...
            for lv in range(self.limit_level + 1):
                ...
                cente = ",".join(
                    [str(self.grid_sizes[lv][i] - 1) for i in range(self.ndims)]
                )
                end = start
            ...
            for lv in range(self.limit_level + 1):
                hfile.write(
                    " ".join([f"{self.dx[lv][i]}" for i in range(self.ndims)]) + "\n"
                )
            # Coordinate system
