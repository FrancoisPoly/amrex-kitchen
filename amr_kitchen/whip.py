...
# Argument parser
parser = argparse.ArgumentParser(description="3D covering grids")

parser.add_argument("plotfile", type=str, help="Path of the plotfile to filter")

parser.add_argument("--variable", "-v", type=str, help="Variable to load")
    ...
    with multiprocessing.Pool() as pool:
        ...
        for res in pool.imap_unordered(readfieldfrombinfile, binfiles[read_order]):
            ...
            for idx, arr in zip(res[0], res[1]):
                data[
                    factor * idx[0][0] : (idx[1][0] + 1) * factor,
                    factor * idx[0][1] : (idx[1][1] + 1) * factor,
                    factor * idx[0][2] : (idx[1][2] + 1) * factor,
                ] = expand_array3d(arr, factor)

np.save(f"{args.variable}_cgrid_{args.plotfile.replace('plt', '')}", data)
