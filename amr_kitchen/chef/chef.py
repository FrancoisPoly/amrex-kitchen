    ...
    with open(args["bfpath"], "rb") as bfr:
        with open(args["newbfpath"], "wb") as bfw:
            while True:
                ...
                newdata = args["recipe"](args["field_indexes"], arr, sarray)
                # Index the required species
                mins.append(np.min(newdata))
                maxs.append(np.max(newdata))
                bfw.write(newdata.flatten(order="F").tobytes())
    ...
    with open(args["bfpath"], "rb") as bfr:
        with open(args["newbfpath"], "wb") as bfw:
            while True:
                ...
                arr = arr.reshape(datashape, order="F")
                # Call the user defined function with the field indexes
                newdata = args["recipe"](args["field_indexes"], arr)
...
class Chef(PlotfileCooker):
    ...
    cookbookhelp = {
        ...
        "user": (
            "A user defined python function named 'recipe'"
    ...
    def __init__(
        ...
        # Error if no recipe can be found
            ...
            raise ValueError(
                (
                    f"The recipe {recipe} is not in the cookbook\n"
        ...
        if self.requires_sol:
            ...
            self.gas, self.P, self.sp_start, self.sp_end = self.sarray_input(
                mech, pressure
            )
    ...
    def cook(self) -> None:
        ...
        for lv in range(self.limit_level + 1):
            ...
            for bfpath in np.unique(level_files):
                ...
                # New binary file
                newbfpath = os.path.join(
                    self.outdir, self.cell_paths[lv], os.path.split(bfpath)[-1]
                )
                call = {
                    "recipe": self.recipe,
                    "bfpath": bfpath,
            ...
            mapped_mins = np.empty(
                (len(self.boxes[lv]), len(self.outfields)), dtype=float
            )
            mapped_maxs = np.empty(
                (len(self.boxes[lv]), len(self.outfields)), dtype=float
            )
            ...
            self.update_cell_header(
                lv,
                os.path.join(self.pfile, self.cell_paths[lv], "Cell_H"),
                mapped_offsets,
                mapped_mins,
                mapped_maxs,
            )
    ...
    def get_user_recipe_info(self) -> tuple[bool, list[str]]:
        ...
        # of positional arguments
            ...
            raise ValueError(
                (
                    f"User supplied recipe in {self.recipe} "
                    " with signature:\n"
    ...
    def sarray_input(self, mech: str, pressure: float) -> tuple[any, float, int, int]:
        ...
        if pressure is None:
            raise ValueError(
                (
                    "Must specify simulation pressure to recreate the"
        ...
        for i, sp in zip(range(species_start, species_end), gas.species()):
            pltspname = field_list[i][2:-1]
            if pltspname != sp.name:
                raise ValueError(
                    (
                        "The order of the species in the Cantera"
